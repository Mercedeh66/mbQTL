...
RegSnp<-function(SnpFile,microbeAbund){
  ...
  n_mycor<-ncol(mycor)
  #mycor_parse<-as.data.frame(mycor[,c(n_snps+1):n_mycor]) #Remove the first cor with snps and keep the rest of cor os snps with taxa as it starts there
  mycor_parse<-as.data.frame(mycor[,seq.int(n_snps+1, n_mycor)]) #Use seqint avoid drama
  mycor_parse2<-as.data.frame(mycor_parse[seq.int(1, n_snps), ])#Remove all the lines after rs ends as you do not need the taxa for row
...
allToAllProduct <- function(SnpFile,microbeAbund, rsID = NULL) {
  ...
  #Function to solve on one of the items
   apply_product<-function(rsID){
     ...
     result <- pivot_longer(mtx, cols=!any_of(c("rsID", "Genus1")),
  ...
  if (!is.null(rsID)){  #If value of rsID is not null do this:
    ...
    result <- lapply(rsids, apply_product) %>% bind_rows() %>%
...
# Deduplicate gene combinations (i.e., "A", "B" == "B", "A", per rsID)
  invprods_and_cors <- invprods_and_cors %>%
    mutate(X1=ifelse(Genus1 < Genus2, Genus1, Genus2),
...
mbQtlCorHeatmap<-function(final_var_long,labels_col=NULL,...){
  ...
  final_var_wide<-final_var2%>%pivot_wider(names_from = Combined_Genus, values_from = rho_xy, 
