...
#' @param SnpFile original snp file with (0,1,2 values for ref, het, alt genotypes), colnames SNP names, rownames, sample IDs.
#' @param selectmicrobe name of the microbe of interest (for example individual significant microbes associate with a snp).
#' @param rsID name of the snp of interest (for example individual significant snps associated with a microbe)
  ...
  function(microbeAbund,SnpFile,cutoff=NULL,selectmicrobe=NULL,rsID,ref = NULL,alt = NULL,het = NULL,color = NULL) {
    ...
    if (!is.null(ref)) {
      final_DF_Logit_plot_cased <- final_DF_Logit_plot %>%
        mutate(genotype = case_when(count == 2 ~ alt,
                                    count == 0 ~ ref,
                                    count == 1 ~ het))
    } else {
      final_DF_Logit_plot_cased <- final_DF_Logit_plot
